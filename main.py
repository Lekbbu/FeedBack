...
async def start(message: types.Message):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			...
			await message.answer('*🔹 Добро пожаловать!*\n\n*🔹 Напиши сюда сообщения и оно будет отправлено администрации бота*\n\n*|| 𝚂𝚙𝚊𝚖-𝙱𝚕𝚘𝚌𝚔𝚒𝚗𝚐 ||*', parse_mode= 'Markdown')
	else:
		await message.answer('🌝')

# Слито в https://t.me/HACKER_PHONE_VIP

@dp.message_handler(content_types=['text'], text='💈Админ меню💈')
async def handfler(message: types.Message, state: FSMContext):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			await message.answer('*🌴Добро пожаловать в админ-меню!*', parse_mode= 'Markdown', reply_markup=kb.adm)
...
async def proc(message: types.Message, state: FSMContext):
	if message.text == '↩️ Отмена':
		...
		id = data.get("uid")
		await state.finish()
		await bot.send_message(id, '<b>🔹 Вам поступил ответ от создателя!</b>\n\n<b>📝Текст:</b> {}'.format(message.text), parse_mode= 'HTML')
